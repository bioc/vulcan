...
vulcan.import <- function(sheetfile, intervals = NULL) {
    ...
    message("Sheet loaded. You have ", nrow(sheet), " samples and ", length(unique(sheet$Condition)), 
        " conditions")
...
#' @export
vulcan.annotate <- function(vobj, lborder = -10000, rborder = 10000, method = c("closest", 
    "strongest", "sum", "topvar", "farthest", "lowvar")) {
    # Annotate (hg19)
    ...
    anno <- annotatePeakInBatch(gr, AnnotationData = annotation, output = "overlapping", 
        FeatureLocForDistance = "TSS", bindingRegion = c(lborder, rborder))
    ...
    # Other methods: they deal with cases where multiple peaks are found
    ...
    # Method sum: when multiple peaks are found, sum their contributions
    if (method == "sum") {
        for (gene in genesmore) {
            subanno <- dfanno[dfanno$feature == gene, ]
    ...
    # Method strongest: when multiple peaks are found, keep the strongest as the
    # representative one
    if (method == "strongest") {
        for (gene in genesmore) {
            subanno <- dfanno[dfanno$feature == gene, ]
    ...
    # Method topvar: when multiple peaks are found, keep the most varying as the
    # representative one
    if (method == "topvar") {
        for (gene in genesmore) {
            subanno <- dfanno[dfanno$feature == gene, ]
    ...
    # Method lowvar: when multiple peaks are found, keep the least varying as the
    # representative one
    if (method == "lowvar") {
        for (gene in genesmore) {
            subanno <- dfanno[dfanno$feature == gene, ]
