...
#' obj$p.value
#' @export
gsea <- function(reflist, set, method = c("permutation", "pareto"), np = 1000, w = 1, 
    gsea_null = NULL) {
    
    # Get elements in set that are in the ref list
    set <- intersect(names(reflist), set)
    
    # Sort the reference list Get the list order, from higher (1) to smaller (n)
    ...
    ### Compute Enrichment Score Compute running sum for hits
    ...
    ### Identify leading edge Create a vector of 0s long as the reference list
    ...
    if (es < 0) {
        ...
        # Case 2: positive ES Define the peak point
    ...
    gsea.obj <- list(es = es, nes = nes, p.value = p.value, ledge = ledge_names, 
        running_score = running_score, set = set, reflist = reflist, inSet = inSet)
    class(gsea.obj) <- "gsea"
    return(gsea.obj)
}
...
null_gsea <- function(set, reflist, w = 1, np = 1000) {
    ...
    gsea_null <- sapply(seq_len(np), function(i) {
        ...
        # And dependending on the cumulative sums, null running sum and null enrichment
        # score
...
plot_gsea <- function(gsea.obj, twoColors = c("red", "blue"), plotNames = FALSE, 
    colBarcode = "black", title = "Running Enrichment Score", correctEntrez = FALSE, 
    bottomYtitle = "List Values", bottomYlabel = "Signature values", ext_nes = NULL, 
    omit_middle = FALSE) {
    ...
    Obs.correl.vector.norm <- (reflist - min.corr)/(max.corr - min.corr) * 1.25 * 
        delta + min.plot
    ...
    plot(0, col = "white", xlim = c(1, N), ylim = c(0, 10), xaxt = "n", yaxt = "n", 
        type = "n", frame.plot = FALSE, xlab = "", ylab = "", xaxs = "r", yaxs = "r", 
        main = paste("Number of elements: ", N, " (in full list), ", length(set), 
            " (in element set)", sep = "", collapse = ""))
    for (position in 1:N) {
        if (inSet[position] == 1) {
            if (N < 50 & length(set) <= 10) {
                rect(xleft = position - 0.2, ybottom = 0, xright = position + 0.2, 
                  ytop = 10, col = colBarcode, border = NA)
            } else {
                abline(v = position, lwd = 2, col = colBarcode)
            }
            if (plotNames) {
                text(labels = names(reflist[position]), x = position - 0.2, y = 0, 
                  srt = 90, offset = 0, pos = 4, font = 2)
            }
        }
    }
    ...
    if (!omit_middle) {
        ...
        plot(ind, running_score, sub = "", xlab = "", ylab = "Enrichment Score", 
            xlim = c(1, N), ylim = c(min.plot, max.plot), type = "l", pch = 20, lwd = 4, 
            cex = 1, col = col.f, xaxs = "r", yaxs = "r", main = title)
        grid(col = "dark grey", lty = 2)
        
        # This is important: zero running score line
        lines(c(1, N), c(0, 0), lwd = 1, lty = 1, cex = 1)
        
        # This is also important: it's the max enrichment vertical line (aka LEADING
        # EDGE)
        lines(c(l.ledge.ref.plot, l.ledge.ref.plot), c(min.plot, max.plot), lwd = 1, 
            lty = 1, cex = 1)
        ...
        if (is.null(ext_nes)) {
            legend(legend_position, legend = c(paste("ES = ", signif(es, 3), sep = ""), 
                paste("NES = ", signif(nes, 3), sep = ""), paste("p-value = ", signif(p.value, 
                  3), sep = "")), bg = "white")
        } else {
            legend(legend_position, legend = c(paste("NES = ", signif(ext_nes, 3), 
                sep = ""), paste("p-value = ", signif(z2p(ext_nes), 3), sep = "")), 
                bg = "white")
        }
    }
    ...
    par(mar = c(2, 4.1, 2, 2.1))
    plot(ind, reflist, type = "l", pch = 20, lwd = 3, xlim = c(1, N), cex = 1, col = 1, 
        xaxs = "r", yaxs = "r", main = bottomMain, ylab = bottomYlabel, cex.axis = 0.8)
    grid(col = "dark grey", lty = 2)
    lines(c(1, N), c(zero.corr.line, zero.corr.line), lwd = 1, lty = 1, cex = 1, 
        col = 1)  # zero correlation horizontal line
    
    if (omit_middle) {
        # If an external NES is not provided, the standard GSEA one is shown
        if (is.null(ext_nes)) {
            legend("top", legend = c(paste("ES = ", signif(es, 3), sep = ""), paste("NES = ", 
                signif(nes, 3), sep = ""), paste("p-value = ", signif(p.value, 3), 
                sep = "")), bg = "white")
        } else {
            legend("top", legend = c(paste("NES = ", signif(ext_nes, 3), sep = ""), 
                paste("p-value = ", signif(z2p(ext_nes), 3), sep = "")), bg = "white")
        }
    }
...
dpareto <- function(x, threshold = 1, exponent, log = FALSE) {
    # Avoid doing limited-precision arithmetic followed by logs if we want the log!
    if (!log) {
...
pareto.fit <- function(data, threshold) {
    ...
    d <- suppressWarnings(ks.test(newdata, ppareto, threshold = threshold, exponent = alpha))
